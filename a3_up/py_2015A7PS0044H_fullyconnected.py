...
# in our particular problem the dimensions are  [number of samples][width][height]
...
Image_train = Image_train.reshape(no_samples_train,width*height).astype('float')
Image_test = Image_test.reshape(no_samples_test,width*height).astype('float')
...
model = Sequential()
model.add(Dense(4096,input_dim = (width*height), activation='sigmoid'))
...
model.fit(Image_train[:sep], Label_train[:sep], validation_data=(Image_train[sep:], Label_train[sep:]), epochs=30, batch_size=200, verbose=2)
